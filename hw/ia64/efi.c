...
static bool ia64_lx_slots_are_movl(uint64_t l_raw, uint64_t x_raw)
{
    (void)l_raw;
    return ((x_raw >> 37) & 0xf) == 0x6;
}

static uint64_t ia64_lx_movl_get_imm64(uint64_t l_raw, uint64_t x_raw)
{
                                        ...
                                        Error **errp)
{
    ...
    if (!ia64_decode_bundle(bundle, &decoded) ||
        ...
        !ia64_lx_slots_are_movl(decoded.slot[1], decoded.slot[2])) {
        error_setg(errp,
    ...
    value = ia64_lx_movl_get_imm64(decoded.slot[1], decoded.slot[2]);
...
static GArray *build_acpi_fadt_table(const VibtaniumFirmwareLayout *layout)
{
    ...
    wr32(fadt + 112, (1 << 0) | (1 << 2) | (1 << 4) |
                       (1 << 5) | (1 << 6) | (1 << 8) |
                       (1 << 10) | (1 << 12));
    write_acpi_gas(fadt + 116, ACPI_ADR_SPACE_SYSTEM_IO, 8, 1,
                   VIBTANIUM_ACPI_RESET_PORT);
    fadt[128] = VIBTANIUM_ACPI_RESET_VALUE;
...
#define EFI_DP_MEDIA_FILE_PATH            0x04
...
static void write_file_path_device_path(uint8_t *blob, size_t size,
                                        uint64_t address,
                                        const char *file_path)
{
    size_t chars = file_path && *file_path ? strlen(file_path) : 0;
    size_t node_length = chars != 0 ? 4 + (chars + 1) * 2 : 0;
    size_t path_length = node_length + EFI_DP_END_LENGTH;
    uint8_t *path = blob_ptr(blob, size, address, path_length);

    if (chars != 0) {
        uint8_t *text = path + 4;

        path = write_device_path_header(path, EFI_DP_MEDIA_DEVICE_PATH,
                                        EFI_DP_MEDIA_FILE_PATH,
                                        node_length);
        for (const char *p = file_path; *p; p++) {
            uint16_t ch = *p == '/' ? '\\' : (uint8_t)*p;
