...
#define EFI_TRANSFER_BOUNCE_SIZE (1024 * 1024)
...
typedef struct EfiLoaderBenchmark {
    bool enabled;
    bool started;
    bool registered;
    int64_t start_us;
    uint64_t guest_read_bytes;
    uint64_t guest_write_bytes;
    uint64_t guest_fill_bytes;
    uint64_t guest_move_bytes;
    uint64_t ram_chunks;
    uint64_t precise_bytes;
    uint64_t file_read_bytes;
    uint64_t block_read_bytes;
    uint64_t load_image_bytes;
    uint64_t media_cache_hits;
    uint64_t media_cache_misses;
    uint64_t media_negative_hits;
    uint64_t image_parses;
} EfiLoaderBenchmark;

typedef struct EfiMediaCacheEntry {
    uint64_t status;
    bool is_directory;
    uint8_t *data;
    size_t size;
...
typedef struct EfiStartedImageFrame {
    EfiCpuFrame cpu;
...
static EfiStartedImageFrame efi_started_images[EFI_MAX_STARTED_IMAGES];
static unsigned efi_started_image_depth;
static EfiChildImage *efi_preboot_drivers[EFI_MAX_LOADED_IMAGES];
static unsigned efi_preboot_driver_count;
static unsigned efi_preboot_driver_next;
...
static bool efi_enter_child_image(CPUIA64State *env, EfiChildImage *child);

static void efi_media_cache_entry_free(gpointer opaque)
{
    EfiMediaCacheEntry *entry = opaque;
...
static void efi_loader_benchmark_dump(void)
{
    EfiLoaderBenchmark *bench = &efi_loader_benchmark;
    int64_t elapsed_us;

    if (!bench->enabled || !bench->started) {
        return;
    }

    elapsed_us = g_get_monotonic_time() - bench->start_us;
    fprintf(stderr, "[ia64-efi-perf] enabled=1\n");
    fprintf(stderr, "[ia64-efi-perf] elapsed_us=%" PRId64 "\n",
            elapsed_us);
#define EFI_BENCH_PRINT(field, name) \
    fprintf(stderr, "[ia64-efi-perf] " name "=%" PRIu64 "\n", \
            bench->field)
    EFI_BENCH_PRINT(guest_read_bytes, "guest.read_bytes");
    EFI_BENCH_PRINT(guest_write_bytes, "guest.write_bytes");
    EFI_BENCH_PRINT(guest_fill_bytes, "guest.fill_bytes");
    EFI_BENCH_PRINT(guest_move_bytes, "guest.move_bytes");
    EFI_BENCH_PRINT(ram_chunks, "guest.ram_chunks");
    EFI_BENCH_PRINT(precise_bytes, "guest.precise_bytes");
    EFI_BENCH_PRINT(file_read_bytes, "file.read_bytes");
    EFI_BENCH_PRINT(block_read_bytes, "block.read_bytes");
    EFI_BENCH_PRINT(load_image_bytes, "load_image.bytes");
    EFI_BENCH_PRINT(media_cache_hits, "media.cache_hits");
    EFI_BENCH_PRINT(media_cache_misses, "media.cache_misses");
    EFI_BENCH_PRINT(media_negative_hits, "media.negative_hits");
    EFI_BENCH_PRINT(image_parses, "load_image.parse_count");
#undef EFI_BENCH_PRINT
}

static bool efi_loader_benchmark_enabled(void)
{
    static int cached = -1;

    if (cached < 0) {
        const char *value = g_getenv("VIBTANIUM_EFI_PERF");

        cached = value != NULL && *value != '\0';
    }
    return cached != 0;
}
...
{
    EfiLoaderBenchmark *bench = &efi_loader_benchmark;
    bool registered = bench->registered;

    if (!efi_loader_benchmark_enabled()) {
        return;
    }
    ...
    bench->enabled = true;
    bench->started = true;
    bench->registered = registered;
    bench->start_us = g_get_monotonic_time();
    if (!bench->registered) {
        atexit(efi_loader_benchmark_dump);
        bench->registered = true;
    }
}

static uint8_t *efi_transfer_bounce_get(size_t requested)
{
    size_t wanted = MIN(requested, (size_t)EFI_TRANSFER_BOUNCE_SIZE);

    if (wanted == 0) {
        wanted = 1;
    }
    if (efi_transfer_bounce_capacity < wanted) {
        efi_transfer_bounce = g_realloc(efi_transfer_bounce, wanted);
        efi_transfer_bounce_capacity = wanted;
    }
    return efi_transfer_bounce;
}
...
static bool efi_gate_is_start_image_return(uint64_t gate_ip)
{
    if (!efi_gate_address_is_valid_alias(gate_ip)) {
        return false;
    }
...
static bool efi_guest_ram_chunk(CPUIA64State *env, uint64_t address,
                                size_t remaining, bool write,
                                hwaddr *paddr, size_t *size)
{
    ...
    if (!QTAILQ_EMPTY(&cs->watchpoints)) {
        return false;
    }

    mmu_idx = ia64_tcg_mmu_index_for_psr(env->psr, false);
    if (!ia64_translate_address_no_detail(
            env, address, write ? MMU_DATA_STORE : MMU_DATA_LOAD,
            mmu_idx, false, &result)) {
        return false;
    }

    page_left = TARGET_PAGE_SIZE - (address & ~TARGET_PAGE_MASK);
    len = MIN(remaining, page_left);
    xlat = result.paddr;
    mr = address_space_translate(&address_space_memory, result.paddr,
                                 &xlat, &len, write,
                                 MEMTXATTRS_UNSPECIFIED);
    if (!memory_region_is_ram(mr) || len == 0) {
        return false;
    }
...
static void efi_guest_read_bulk(CPUIA64State *env, uint64_t address,
                                void *buffer, size_t size)
{
    ...
    while (done < size) {
        ...
        if (efi_guest_ram_chunk(env, address + done, size - done, false,
                                &paddr, &chunk) &&
            address_space_read(&address_space_memory, paddr,
                               MEMTXATTRS_UNSPECIFIED, bytes + done,
                               chunk) == MEMTX_OK) {
            if (bench->enabled) {
                bench->ram_chunks++;
            }
        ...
        bytes[done] = efi_guest_ldb(env, address + done);
        if (bench->enabled) {
            bench->precise_bytes++;
        }
        done++;
...
static void efi_guest_write_bulk(CPUIA64State *env, uint64_t address,
                                 const void *buffer, size_t size)
{
    ...
    while (done < size) {
        ...
        if (efi_guest_ram_chunk(env, address + done, size - done, true,
                                &paddr, &chunk) &&
        ...
        efi_guest_stb(env, address + done, bytes[done]);
        if (bench->enabled) {
            bench->precise_bytes++;
        }
        done++;
...
static void efi_guest_fill_bulk(CPUIA64State *env, uint64_t address,
                                uint8_t value, size_t size)
{
    ...
    while (done < size) {
        ...
        if (efi_guest_ram_chunk(env, address + done, size - done, true,
                                &paddr, &chunk) &&
            address_space_set(&address_space_memory, paddr, value, chunk,
        ...
        efi_guest_stb(env, address + done, value);
        if (bench->enabled) {
            bench->precise_bytes++;
        }
        done++;
...
static void efi_guest_move_bulk(CPUIA64State *env, uint64_t destination,
                                uint64_t source, size_t size)
{
    EfiLoaderBenchmark *bench = &efi_loader_benchmark;
    uint8_t *bounce = efi_transfer_bounce_get(size);
    size_t capacity = MIN(size, efi_transfer_bounce_capacity);

    if (bench->enabled) {
        bench->guest_move_bytes += size;
    }
    ...
    if (destination < source || destination - source >= size) {
        ...
        while (done < size) {
            size_t chunk = MIN(capacity, size - done);

            efi_guest_read_bulk(env, source + done, bounce, chunk);
            efi_guest_write_bulk(env, destination + done, bounce, chunk);
            done += chunk;
        }
        return;
    }
    ...
    while (size != 0) {
        size_t chunk = MIN(capacity, size);
        size_t offset = size - chunk;

        efi_guest_read_bulk(env, source + offset, bounce, chunk);
        efi_guest_write_bulk(env, destination + offset, bounce, chunk);
        size = offset;
    }
}
...
static void efi_guest_read_guid(CPUIA64State *env, uint64_t address,
                                uint8_t guid[16])
{
    efi_guest_read_bulk(env, address, guid, 16);
}
                                 ...
                                 const char *guid)
{
    ...
    efi_guest_write_bulk(env, address, bytes, sizeof(bytes));
}
                                  ...
                                  unsigned media_count)
{
    ...
    g_clear_pointer(&efi_media_files, g_hash_table_unref);
    ...
    memset(efi_started_images, 0, sizeof(efi_started_images));
    efi_started_image_depth = 0;
    memset(efi_preboot_drivers, 0, sizeof(efi_preboot_drivers));
    efi_preboot_driver_count = 0;
    efi_preboot_driver_next = 0;
...
static uint64_t efi_calculate_crc32(CPUIA64State *env)
{
    ...
    efi_guest_read_bulk(env, data, bytes, size);
...
static uint64_t efi_copy_mem(CPUIA64State *env)
{
    ...
    efi_guest_move_bulk(env, destination, source, length);
    return VIBTANIUM_EFI_SUCCESS;
}
...
static uint64_t efi_set_mem(CPUIA64State *env)
{
    ...
    efi_guest_fill_bulk(env, buffer, value, size);
    return VIBTANIUM_EFI_SUCCESS;
}
...
static EfiMediaCacheEntry *efi_media_cache_open(const char *storage_path)
{
    EfiLoaderBenchmark *bench = &efi_loader_benchmark;
    EfiMediaCacheEntry *entry;
    g_autofree char *key = g_ascii_strdown(storage_path, -1);
    ...
    entry = g_hash_table_lookup(efi_media_files, key);
    if (entry) {
        if (bench->enabled) {
            bench->media_cache_hits++;
            if (entry->status != VIBTANIUM_EFI_SUCCESS) {
                bench->media_negative_hits++;
            }
        }
        return entry;
    }
    ...
    if (bench->enabled) {
        bench->media_cache_misses++;
    }
    entry = g_new0(EfiMediaCacheEntry, 1);
    entry->report.status = VIBTANIUM_EFI_STORAGE_NOT_FOUND;
    g_strlcpy(entry->report.message, "path has not been opened",
              sizeof(entry->report.message));
    if (vibtanium_efi_media_open_path(
            &efi_boot_media, storage_path, &entry->is_directory,
            &entry->data, &entry->size, entry->source,
            sizeof(entry->source), &entry->report, &local_err)) {
        entry->status = VIBTANIUM_EFI_SUCCESS;
    } else {
        entry->status = efi_storage_status_to_efi(entry->report.status);
    }
    error_free(local_err);
    g_hash_table_insert(efi_media_files, g_steal_pointer(&key), entry);
    return entry;
}
...
static void efi_write_loaded_image_protocol(CPUIA64State *env,
                                            ...
                                            uint64_t file_path)
{
    efi_guest_fill_bulk(env, address, 0, 96);
...
static uint64_t efi_register_child_image(CPUIA64State *env,
                                         uint64_t parent_handle,
                                         ...
                                         EfiChildImage **child_out)
{
    ...
    if (load_options_size > UINT32_MAX ||
        (load_options_size != 0 && !load_options)) {
        return VIBTANIUM_EFI_INVALID_PARAMETER;
    }
    if (load_options_size != 0) {
        options = efi_pool_alloc_raw(load_options_size);
        if (options == 0) {
            return VIBTANIUM_EFI_OUT_OF_RESOURCES;
        }
        efi_guest_write_bulk(env, options, load_options, load_options_size);
    }

    pages = QEMU_ALIGN_UP(loaded->size, EFI_PAGE_SIZE) / EFI_PAGE_SIZE;
    if (loaded->relocation_rva == 0 || loaded->relocation_size == 0) {
        load_base = loaded->load_base;
    } else {
        if (!efi_find_free_pages(pages, efi_ram_top() - 1, &load_base)) {
            return VIBTANIUM_EFI_OUT_OF_RESOURCES;
        }
        if (!vibtanium_efi_image_relocate(loaded, load_base, &local_err)) {
            if (efi_trace_enabled()) {
    ...
    efi_guest_write_bulk(env, loaded->load_base, loaded->data, loaded->size);
    if (efi_loader_benchmark.enabled) {
        efi_loader_benchmark.load_image_bytes += loaded->size;
    }
    child->pages = pages;
    child->file_path = device_path;
    child->image = *loaded;
    memset(loaded, 0, sizeof(*loaded));
    g_strlcpy(child->path, path && path[0] ? path : child->image.source_path,
              sizeof(child->path));
    efi_write_loaded_image_protocol(env, child->loaded_image, &child->image,
                                    parent_handle, child->file_path);

    if (options != 0) {
        efi_guest_stl(env, child->loaded_image + 48, load_options_size);
        efi_guest_stq(env, child->loaded_image + 56, options);
    }
                                           ...
                                           size_t path_size)
{
    ...
    if (source_buffer != 0 || source_size != 0) {
        ...
        *file_data = g_malloc(source_size);
        efi_guest_read_bulk(env, source_buffer, *file_data, source_size);
...

    {
        ...
        EfiMediaCacheEntry *cached;

        g_snprintf(storage_path, sizeof(storage_path), "/%s", path);
        cached = efi_media_cache_open(storage_path);
        if (!cached || cached->status != VIBTANIUM_EFI_SUCCESS ||
            cached->is_directory) {
            uint64_t status = cached ? cached->status
                                     : VIBTANIUM_EFI_NOT_FOUND;
            const VibtaniumEfiStorageReport *report =
                cached ? &cached->report : NULL;

            if (efi_trace_enabled()) {
                fprintf(stderr,
                        "[efi-load-image] path='%s' status=%s reason=%s\n",
                        storage_path,
                        report ? vibtanium_efi_storage_status_name(
                                     report->status) : "not-found",
                        report ? report->message : "media cache unavailable");
            }
            return cached && cached->is_directory
                ? VIBTANIUM_EFI_LOAD_ERROR : status;
        }
        *file_data = g_memdup2(cached->data, cached->size);
        *file_size = cached->size;
        g_strlcpy(source, cached->source, source_size_chars);
    }
...
static uint64_t efi_load_image(CPUIA64State *env)
{
    ...
    if (efi_loader_benchmark.enabled) {
        efi_loader_benchmark.image_parses++;
    }
    ...
    status = efi_register_child_image(env, parent_handle, device_path,
                                      &loaded, path[0] ? path : source,
                                      NULL, 0, &child);
    if (status != VIBTANIUM_EFI_SUCCESS) {
...
static void efi_save_cpu_frame(EfiCpuFrame *frame, CPUIA64State *env)
{
    ...
    frame->pr = env->pr;
    memcpy(frame->br, env->br, sizeof(frame->br));
    memcpy(frame->ar, env->ar, sizeof(frame->ar));
    memcpy(frame->cr, env->cr, sizeof(frame->cr));
    memcpy(frame->rr, env->rr, sizeof(frame->rr));
    memcpy(frame->dbr, env->dbr, sizeof(frame->dbr));
    memcpy(frame->ibr, env->ibr, sizeof(frame->ibr));
    memcpy(frame->pkr, env->pkr, sizeof(frame->pkr));
    memcpy(frame->cpuid, env->cpuid, sizeof(frame->cpuid));
    memcpy(frame->itr, env->itr, sizeof(frame->itr));
    memcpy(frame->dtr, env->dtr, sizeof(frame->dtr));
    memcpy(frame->pmc, env->pmc, sizeof(frame->pmc));
...
static void efi_restore_cpu_frame(CPUIA64State *env,
                                  const EfiCpuFrame *frame)
{
    memcpy(env->gr, frame->gr, sizeof(env->gr));
    memcpy(env->banked_gr, frame->banked_gr, sizeof(env->banked_gr));
    memcpy(env->fr, frame->fr, sizeof(env->fr));
    env->pr = frame->pr;
    memcpy(env->br, frame->br, sizeof(env->br));
    memcpy(env->ar, frame->ar, sizeof(env->ar));
    memcpy(env->cr, frame->cr, sizeof(env->cr));
    memcpy(env->rr, frame->rr, sizeof(env->rr));
    memcpy(env->dbr, frame->dbr, sizeof(env->dbr));
    memcpy(env->ibr, frame->ibr, sizeof(env->ibr));
    memcpy(env->pkr, frame->pkr, sizeof(env->pkr));
    memcpy(env->cpuid, frame->cpuid, sizeof(env->cpuid));
    memcpy(env->itr, frame->itr, sizeof(env->itr));
    memcpy(env->dtr, frame->dtr, sizeof(env->dtr));
...
/*
 * A host-dispatched EFI transition starts with its own empty register-stack
 * context.  The caller's complete RSE image has already been saved and will
 * be restored at the matching return gate.  Reinitializing every partition
 * here avoids grafting a firmware callback onto the application's live frame.
 */
static void efi_prepare_fresh_rse_context(CPUIA64State *env)
{
    memset(&env->rse, 0, sizeof(env->rse));
    ...
    /* First make CFM zero, then let ia64_set_cfm adjust pristine partitions. */
    ia64_set_cfm(env, 0);
    env->ar[IA64_AR_RSC] = 0;
...
static bool efi_return_from_child_image(CPUIA64State *env, uint64_t status,
                                        const char *reason)
{
    EfiStartedImageFrame frame;

    if (efi_started_image_depth == 0) {
        warn_report("vibtanium EFI child image return with no active image");
        return false;
    }

    frame = efi_started_images[--efi_started_image_depth];
    memset(&efi_started_images[efi_started_image_depth], 0,
           sizeof(efi_started_images[efi_started_image_depth]));

    warn_report("vibtanium EFI StartImage returned handle=0x%016" PRIx64
                " status=0x%016" PRIx64 " reason=%s path=%s",
                frame.image_handle, status, reason ? reason : "return",
                frame.path);

    efi_restore_cpu_frame(env, &frame.cpu);
    ia64_write_gr(env, 8, status);
    if (frame.preboot_driver) {
        while (efi_preboot_driver_next < efi_preboot_driver_count) {
            EfiChildImage *next =
                efi_preboot_drivers[efi_preboot_driver_next++];

            if (efi_enter_child_image(env, next)) {
                efi_started_images[efi_started_image_depth - 1]
                    .preboot_driver = true;
                return true;
            }
            warn_report("vibtanium EFI DriverOrder entry %s failed to start",
                        next->path);
        }
        return true;
    }
    ia64_return_from_call_frame(env, frame.parent_return_ip);
    return true;
}
...
static bool efi_enter_child_image(CPUIA64State *env, EfiChildImage *child)
{
    ...
    if (efi_started_image_depth >= EFI_MAX_STARTED_IMAGES) {
        warn_report("vibtanium EFI StartImage nesting limit reached");
        return false;
    }
    ...
    frame->image_handle = child->handle;
    frame->parent_return_ip = env->br[0];
    frame->exit_data_size = ia64_read_gr(env, 33);
    frame->exit_data = ia64_read_gr(env, 34);
    g_strlcpy(frame->path, child->path, sizeof(frame->path));
    ...
    env->pr = 1;
    efi_prepare_fresh_rse_context(env);
                                      ...
                                      Error **errp)
{
    ...
    if (!env || !file_data || file_size == 0 ||
        efi_preboot_driver_count >= ARRAY_SIZE(efi_preboot_drivers)) {
        error_setg(errp, "invalid or excessive EFI DriverOrder image");
        return false;
    }
    ...
    status = efi_register_child_image(env, VIBTANIUM_EFI_IMAGE_HANDLE, 0,
                                      &loaded, path, load_options,
                                      load_options_size, &child);
    vibtanium_efi_image_destroy(&loaded);
    if (status != VIBTANIUM_EFI_SUCCESS) {
        error_setg(errp, "could not register EFI DriverOrder image: %s",
                   vibtanium_efi_status_name(status));
        return false;
    }

    efi_preboot_drivers[efi_preboot_driver_count++] = child;
    warn_report("vibtanium EFI DriverOrder queued path=%s handle=0x%016" PRIx64,
                child->path, child->handle);
    return true;
}
...
bool vibtanium_efi_start_driver_images(CPUIA64State *env)
{
    ...
    if (!env || efi_preboot_driver_count == 0) {
        return false;
    }

    efi_preboot_driver_next = 1;
    first = efi_preboot_drivers[0];
    if (!efi_enter_child_image(env, first)) {
        return false;
    }
    efi_started_images[efi_started_image_depth - 1].preboot_driver = true;
    return true;
}
...
static uint64_t efi_exit(CPUIA64State *env)
{
    uint64_t image_handle = ia64_read_gr(env, 32);
    uint64_t status = ia64_read_gr(env, 33);
    uint64_t exit_data_size = ia64_read_gr(env, 34);
    uint64_t exit_data = ia64_read_gr(env, 35);
    EfiStartedImageFrame *frame;
    ...
    frame = &efi_started_images[efi_started_image_depth - 1];
    if (frame->image_handle != image_handle) {
        return VIBTANIUM_EFI_INVALID_PARAMETER;
    }

    if (frame->exit_data_size != 0) {
        efi_guest_stq(env, frame->exit_data_size, exit_data_size);
    }
    if (frame->exit_data != 0) {
        efi_guest_stq(env, frame->exit_data, exit_data);
    }

    efi_pending_exit_image = true;
    efi_pending_exit_status = status;
    return VIBTANIUM_EFI_SUCCESS;
}
...
static uint64_t efi_file_open(CPUIA64State *env)
{
    ...
    cached = efi_media_cache_open(storage_path);
    if (!cached || cached->status != VIBTANIUM_EFI_SUCCESS) {
        uint64_t status = cached ? cached->status : VIBTANIUM_EFI_NOT_FOUND;
        const VibtaniumEfiStorageReport *report =
            cached ? &cached->report : NULL;

        trace_ia64_efi_file_open_result(
            env->ip, storage_path,
            report ? vibtanium_efi_storage_status_name(report->status)
                   : "not-found",
            0, 0, report ? report->message : "media cache unavailable");
        if (efi_trace_enabled()) {
    ...
    if (efi_trace_enabled()) {
        fprintf(stderr, "[efi-file] open path='%s' size=0x%" PRIx64
                " source=%s\n",
                storage_path, (uint64_t)cached->size, cached->source);
    }

    file = efi_create_file_node(env, path, false, cached->data, cached->size);
    if (!file) {
        efi_guest_stq(env, new_handle_out, 0);
        return VIBTANIUM_EFI_OUT_OF_RESOURCES;
    }
...
static uint64_t efi_file_read(CPUIA64State *env)
{
    ...
    if (actual > 0) {
        ...
        efi_guest_write_bulk(env, buffer, file->data + file->position,
                             actual);
        if (efi_loader_benchmark.enabled) {
            efi_loader_benchmark.file_read_bytes += actual;
        }
...
static uint64_t efi_file_get_info(CPUIA64State *env)
{
    ...
    efi_guest_fill_bulk(env, buffer, 0, required);
...
static uint64_t efi_dispatch_block_io(CPUIA64State *env, unsigned index)
{
    ...
    case 1: {
        ...
        while (done < buffer_size) {
            ...
            efi_guest_write_bulk(env, buffer + done, data, chunk);
            done += chunk;
        }
        if (efi_loader_benchmark.enabled) {
            efi_loader_benchmark.block_read_bytes += buffer_size;
        }
        return VIBTANIUM_EFI_SUCCESS;
    }
    case 2: {
...
static uint64_t efi_dispatch_boot_service(CPUIA64State *env, unsigned index)
{
    ...
    case VIBTANIUM_EFI_BOOT_EXIT_INDEX:
        return efi_exit(env);
...
static uint64_t efi_runtime_get_variable(CPUIA64State *env)
{
    ...
    if (data_size != 0) {
        efi_guest_write_bulk(env, data_ptr, data, data_size);
    }
    return VIBTANIUM_EFI_SUCCESS;
}
...
static uint64_t efi_runtime_set_variable(CPUIA64State *env)
{
    ...
    if (data_size != 0) {
        ...
        efi_guest_read_bulk(env, data_ptr, data, data_size);
    }
...
bool vibtanium_efi_dispatch_gate(CPUIA64State *env, uint64_t gate_ip)
{
    ...
    if (efi_gate_is_start_image_return(gate_ip)) {
    ...
    if (efi_pending_exit_image) {
        uint64_t exit_status = efi_pending_exit_status;

        efi_pending_exit_image = false;
        efi_pending_exit_status = VIBTANIUM_EFI_SUCCESS;
        ia64_write_gr(env, 8, result);
        efi_trace_service(env, service_index, result);
        return efi_return_from_child_image(env, exit_status, "Exit");
    }

    if (result == VIBTANIUM_EFI_SUCCESS && efi_pending_start_image) {
        EfiChildImage *child = efi_pending_start_image;
