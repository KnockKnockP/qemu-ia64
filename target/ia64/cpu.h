...
#define IA64_RID_BITS 18
#define IA64_RID_MASK ((UINT64_C(1) << IA64_RID_BITS) - 1)
#define IA64_RR_VE_BIT UINT64_C(0x1)
#define IA64_RR_PS_SHIFT 2
#define IA64_RR_PS_MASK (UINT64_C(0x3f) << IA64_RR_PS_SHIFT)
#define IA64_RR_RID_SHIFT 8
#define IA64_RR_RID_MASK (IA64_RID_MASK << IA64_RR_RID_SHIFT)
#define IA64_RR_IMPLEMENTED_MASK \
    (IA64_RR_VE_BIT | IA64_RR_PS_MASK | IA64_RR_RID_MASK)
/* SDM Vol. 2, Table 4-5 and PAL_VM_PAGE_SIZE. */
#define IA64_INSERTABLE_PAGE_SIZE_MASK \
    ((UINT64_C(1) << 12) | (UINT64_C(1) << 13) | \
     (UINT64_C(1) << 14) | (UINT64_C(1) << 16) | \
     (UINT64_C(1) << 18) | (UINT64_C(1) << 20) | \
     (UINT64_C(1) << 22) | (UINT64_C(1) << 24) | \
     (UINT64_C(1) << 26) | (UINT64_C(1) << 28))
#define IA64_PURGEABLE_PAGE_SIZE_MASK \
    (IA64_INSERTABLE_PAGE_SIZE_MASK | (UINT64_C(1) << 32))
...
#define IA64_CPUID_FEATURE_LB (UINT64_C(1) << 0)
#define IA64_CPUID_FEATURE_CZ (UINT64_C(1) << 32)
#define IA64_CPUID_FEATURE_X2 (UINT64_C(1) << 33)
...
#define IA64_TB_FLAG_PSR_ONESHOT (1u << 17)
...
static inline uint32_t ia64_tcg_tb_flags_from_psr(uint64_t psr)
{
    ...
    if (psr & (IA64_PSR_ED_BIT | IA64_PSR_FAULT_SUPPRESSION_MASK)) {
...
typedef struct IA64IssueGroupState {
    ...
    /* GR results explicitly forwarded by a successful check-load miss. */
