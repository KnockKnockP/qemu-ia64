...
#define FAT12_VOLUME_BYTES (FAT12_TOTAL_SECTORS * 512)
...
static void write_fat12_entry(uint8_t *fat, uint16_t cluster, uint16_t value)
{
    ...
    uint16_t current = fat[offset] | (fat[offset + 1] << 8);
...
static void make_fat12_volume(uint8_t *image, size_t image_size)
{
    uint8_t *fat;
    uint8_t *root;
    uint8_t *efi_dir;
    ...
    uint32_t root_offset = (1 + 1) * 512;
    uint32_t data_offset = (1 + 1 + 1) * 512;

    g_assert_cmpuint(image_size, >=, FAT12_VOLUME_BYTES);
    memset(image, 0, FAT12_VOLUME_BYTES);
    ...
    store_le16(image + 19, FAT12_TOTAL_SECTORS);
    image[21] = 0xf8;
    store_le16(image + 22, 1);
    image[510] = 0x55;
    image[511] = 0xaa;
    ...
    fat[1] = 0xff;
    fat[2] = 0xff;
    write_fat12_entry(fat, 2, 0x0fff);
    write_fat12_entry(fat, 3, 0x0fff);
    write_fat12_entry(fat, 4, 0x0fff);
    write_fat12_entry(fat, 5, 0x0fff);

    root = image + root_offset;
    write_fat_entry(root, 0, "EFI        ", 0x10, 2, 0);
    write_fat_entry(root, 1, "SETUPLDREFI", 0x20, 5, 4);
    ...
    memcpy(image + data_offset + 3 * 512, "SETU", 4);
}
...
static void make_eltorito_fat12_iso(MemoryIso *iso)
{
    uint8_t *pvd;
    uint8_t *boot_record;
    uint8_t *catalog;
    ...
    catalog[0x20] = 0x88;
    catalog[0x21] = 0x00;
    store_le16(catalog + 0x26, 1);
    store_le32(catalog + 0x28, BOOT_IMAGE_SECTOR);

    image = iso->bytes + image_base;
    make_fat12_volume(image, iso->size - image_base);
}
...
static void test_cdrom_read_path_uses_eltorito_fat12(void)
{
    MemoryIso iso;
    VibtaniumEfiBlockDevice dev;
    VibtaniumEfiStorageReport report;
    ...
    g_assert_cmpmem(data, size, "IA64", 4);
    g_assert_nonnull(strstr(source, "eltorito"));
    g_assert_nonnull(strstr(report.message, "FAT12"));

    g_clear_pointer(&data, g_free);
    size = 0;
    memset(source, 0, sizeof(source));
    g_assert_true(vibtanium_efi_cdrom_read_path(
                      &dev, "/SETUPLDR.EFI", &data, &size, source,
                      sizeof(source), &report, &err));
    g_assert_null(err);
    g_assert_cmpuint(size, ==, 4);
    g_assert_cmpmem(data, size, "SETU", 4);
    g_assert_nonnull(strstr(source, "eltorito"));
    g_assert_nonnull(strstr(report.message, "FAT12"));
}
...
int main(int argc, char **argv)
{
    ...
    g_test_add_func("/ia64-efi-storage/cdrom-read-path-uses-eltorito-fat12",
                    test_cdrom_read_path_uses_eltorito_fat12);
